...
function draw() {
 ...
 switch(score){
  ...
  if(keyDown("space") &&  Nobita.y>=100) {
   ...
   if( Nobita_running !== undefined){
   var index=0;
   var x;
   var y;
    for(var plr in Nobita_running){
     index=index+1;
     x=x+200;
     y=displayHeight-Nobita_running[plr].distance;
     Nobita_running[index-1];
     Nobita_running[index-1];
     if(index===Nobita_running.index){
       camera.position.x=displayWidth/2;
       camera.position.y=Nobita_running[index-1].y;
     }
    }
  }
